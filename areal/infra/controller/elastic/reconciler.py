...
class RolloutInstanceReconciler:
    ...
    def _instances(self) -> list[RolloutInstance]:
        return [
            self._pool.get(instance_id) for instance_id in self._pool.instance_ids()
        ]

    @staticmethod
    def _is_running(instance: RolloutInstance) -> bool:
        return (
    ...
    async def reconcile_once(self) -> ReconcileResult:
        ...
        for instance in instances:
            if (
                ...
                await self._ensure_proxy(instance)
        running = [instance for instance in instances if self._is_running(instance)]
        draining_instances = [
            instance
            for instance in instances
        ...
        existing_running = sum(
            instance.instance_id not in started_ids and self._is_running(instance)
