...
def test_online_sessions_participate_in_least_loaded_capacity():
    pool = _pool()
    first = _add_ready(pool, "ri-first")
    _add_ready(pool, "ri-second")
    pool.acquire_session(first.instance_id)

    target = pool.reserve_task("task-a", max_inflight_per_instance=1)
